...
import type { InjectionTrigger, ProvideContext } from '../../injection/types.js'
...
export interface ContextInjectorConfig {
   ...
   * default — or `'everyTurn'`); a predicate over the {@link ProvideContext} is the escape hatch. A
   * predicate that throws fails open (injection is skipped).
   *
   * @defaultValue 'userTurn'
   */
  trigger?: InjectionTrigger | ((context: ProvideContext) => boolean)
  /**
   * Returns the text to fold into the latest user message for this call, or `undefined`/`''` to skip.
   ...
   */
  provide: (context: ProvideContext) => Promise<string | undefined>
  /**
 ...
 *
 * On a model call, the plugin asks {@link ContextInjectorConfig.provide} for text and folds it into
 * the most recent user message (ahead of the user's own content), gated by
 ...
 *   model,
 *   plugins: [new ContextInjector({ provide: async () => `<now>${new Date().toISOString()}</now>` })],
 * })
 * ```
 *
 * @remarks
