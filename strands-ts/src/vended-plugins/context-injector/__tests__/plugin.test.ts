...
describe('ContextInjector', () => {
  describe('plugin interface', () => {
    it('defaults to the strands:context-injector name', () => {
      expect(new ContextInjector({ provide: async () => 'x' }).name).toBe('strands:context-injector')
    })

    it('honors a custom name (so multiple injectors can be told apart)', () => {
      expect(new ContextInjector({ name: 'now', provide: async () => 'x' }).name).toBe('now')
    })

    it('registers an InvokeModelStage input middleware on initAgent', () => {
      const { agent, addMiddleware } = makeAgent()
      new ContextInjector({ provide: async () => 'x' }).initAgent(agent)
  ...
  describe('registered handler', () => {
    ...
    it('folds provide() text into the latest user message', async () => {
      const result = await run(new ContextInjector({ provide: async () => 'INJECTED' }), [
        assistant('prior'),
        user('ask'),
      ])
    ...
    it('skips on a non-user turn by default (userTurn trigger)', async () => {
      ...
      const result = await run(new ContextInjector({ provide }), input)
      expect(provide).not.toHaveBeenCalled()
      expect(result.messages).toBe(input)
    })

    it("'everyTurn' injects regardless of the latest role", async () => {
      const result = await run(new ContextInjector({ trigger: 'everyTurn', provide: async () => 'INJECTED' }), [
        user('ask'),
        assistant('reply'),
      ])
    ...
    it('exposes appState and the cancel signal to provide', async () => {
      const { agent, addMiddleware } = makeAgent()
      let sawSignal = false
    ...
    it('fails open (passes context through) when provide throws', async () => {
