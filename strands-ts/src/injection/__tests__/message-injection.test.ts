...
import type { ProvideContext } from '../types.js'
...
// resolveTrigger predicates take a ProvideContext; tests only exercise `messages`, so a minimal bag suffices.
const provideCtx = (messages: MessageData[]) => ({ messages }) as unknown as ProvideContext
...
describe('resolveTrigger', () => {
  it('defaults (undefined) to the userTurn policy', () => {
    ...
    expect(trigger(provideCtx([user('ask').toJSON()]))).toBe(true)
    expect(trigger(provideCtx([toolResult().toJSON()]))).toBe(false)
  })
  ...
  it("'everyTurn' always fires", () => {
    ...
    expect(trigger(provideCtx([]))).toBe(true)
    expect(trigger(provideCtx([toolResult().toJSON()]))).toBe(true)
  })

  it('uses a custom predicate over the context', () => {
    const trigger = resolveTrigger((context) => context.messages.length >= 2)
    expect(trigger(provideCtx([user('a').toJSON()]))).toBe(false)
    expect(trigger(provideCtx([user('a').toJSON(), assistant('b').toJSON()]))).toBe(true)
  })
  ...
  it('fails open (returns false, logs) when a custom predicate throws', () => {
    ...
    expect(trigger(provideCtx([user('ask').toJSON()]))).toBe(false)
...
describe('createInjectionMiddleware', () => {
  ...
  // ProvideContext (appState/signal) from `context.agent`, then spreads the rest through, so a context
  ...
  it('passes a ProvideContext carrying the conversation (as data) to provide', async () => {
    const seen: string[] = []
  ...
  it('exposes appState and the cancel signal on the ProvideContext', async () => {
